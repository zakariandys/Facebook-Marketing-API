...
public class BatchMode {
	...
	public static void main(String[] args) {
		try {
			...
			account.createCampaign().setName("Budi Luhur")
					.setObjective(Campaign.EnumObjective.VALUE_LINK_CLICKS)
					.setStatus(Campaign.EnumStatus.VALUE_PAUSED).addToBatch(batch, "campaignRequest");
			account.createAdSet().setName("Adset Test").setCampaignId("{result=campaignRequest:$.id}")
			...
			account.createAdCreative().setTitle("Creative Ad Test").setBody("Java SDK Batch Test Creative")
			...
			account.createAd().setName("Ad Test").setAdsetId("{result=adsetRequest:$.id}")
